...
Our opening: we can compute OBSERVED lead time and diff it against the contracted lead time that nobody has updated since go-live. Same for MOQ drift. Cheap SQL, no ERP does it, and it explains to the customer WHY their safety stock is wrong.

**CORRECTION (2026-09-04).** An earlier version of this paragraph said "we already hold fact_procurement, we can compute observed lead time (PO date to receipt date)". That is **not true against the actual schema**: `fact_procurement` carries `ORDER_DATE_KEY`, `EXPECTED_DATE_KEY`, `RECEIVED_QTY`, `PENDING_QTY` and `STATUS` — but **no actual-receipt date**, so it can measure a *promise* and never a *performance*. Observed lead time comes from `fact_supplier_delivery` instead (`PLANNED_DATE_KEY` vs `DELIVERY_DATE_KEY`, plus `DELAY_DAYS`), which is a weaker source in one specific way: it has no `PART_KEY`, so lead time is only available per (supplier, warehouse) unless that column is added. `fact_procurement`'s real role is the inbound pipeline (what is on order and when it is expected). See `docs/schema_changes_gold_dev_analytics.md` §1.1–1.2 — both gaps are open requests to Data Engineering.
...
| 5 | Lead-time reality check (observed vs. contracted) | 73% of AI initiatives fail on data quality; stale master data causes both false shortages and false comfort (Section 4) | Low build cost — `fact_supplier_delivery` has planned vs actual delivery dates (**not** `fact_procurement`, which has no receipt date — see the correction in Section 4). Explains WHY the customer's safety stock is wrong, not just that it is | Diff observed delivery performance against the contracted lead time |
