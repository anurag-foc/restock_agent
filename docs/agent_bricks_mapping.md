...
- **Tasks (two, linear — no condition task):**
  ...
  2. `invoke_supervisor` (`notebooks/lakeflow_trigger/invoke_supervisor.py`) — `COUNT(*)` over `rank_priority_actions(5)`; `dbutils.notebook.exit("NO_ACTION")` at zero, otherwise the 2+N-turn conversation of §2.5 (N = distinct live signal types, typically 1-3).
...
**The 290s ceiling and the 2+N-turn protocol.** Model Serving cuts the HTTP connection at ~290s; the notebook sets a 280s per-turn timeout. Each request resets the clock. The protocol is no longer a fixed three turns — it surfaces the top-ranked action for EACH distinct signal type live this run (`rank_priority_actions_diverse`, typically 1-3: `STOCK_THRESHOLD`, `BOM_CASCADE_RISK`, `STALLED_COMMITMENT`), bundled into one quote and one Teams card rather than the single loudest number:

- **Pre-check.** `COUNT(*)` over `rank_priority_actions_diverse()` gives `N`, the number of distinct signal types with a live candidate this run — a count, not a judgment, same category as the existing `rank_priority_actions(5)` NO_ACTION gate.
- **Turn 1 — rank.** One Genie call to `rank_priority_actions_diverse`; list the top action per signal type, verbatim, nothing else.
- **Turns 2.1 .. 2.N — analyse.** One round-trip PER candidate (its own 280s budget): one or two drill-down Genie calls (`scan_*`/`evaluate_*`) most relevant to that candidate's `signal_type`, plus `scan_demand_shift` and `scan_leadtime_drift` **always** (corrections to burn rate and lead time that every other figure rests on — cite their result, or their absence, in an EVIDENCE line even when they return nothing), then a resolution (transfer / PO / escalation with no action) written up in the OUTPUT CONTRACT format tagged `## CANDIDATE i of N`, stating the cost of acting *and* of doing nothing.
- **Final turn — act.** `persist_quote` with all `N` candidates in one array (one `quote_id`, one line per candidate), then `send_human_review` once.

A turn that ranks, drills into a candidate, AND writes it up in one round-trip reliably blows the ceiling — that's why the ranking turn, each candidate's analysis, and the final persist/notify turn are three separate kinds of round-trip, with N of the middle kind (one per live signal type). Doing N candidates as N separate round-trips costs wall-clock on a day with more live signal types, never more timeout risk — do not merge them back into fewer, bigger turns.
