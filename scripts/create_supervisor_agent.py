...
RESTOCK_MAKER_TOOL_DESCRIPTION = (
    "Restock Request Maker — a fulfillment-time guardrail for a restock line a Production "
    ...
    "VARIANCE_QTY itself; do not ask Restock Request Maker for a number and do not pass one along "
    "yourself.\n\n"
...
ACTIONS_TOOL_DESCRIPTION = (
    ...
    "2. `send_human_review(quote_id, summary_report, review_url)` — send the Production Manager "
    "a Microsoft Teams card with a deep link to the Databricks Review App. Call this only after "
    "persist_quote has returned a quote_id, using that exact id.\n"
    "3. `fulfill_restock_request(restock_request_key, proceed, note)` — record your PROCEED/"
    "NEEDS_REVIEW verdict on a single APPROVED line, after asking Restock Request Maker. This "
    ...
    "duplicating a quote, a Teams card, or a status transition."
)
...
SUPERVISOR_INSTRUCTIONS = (
    ...
    "Restock Request Maker) never write.\n\n"
    ...
    "2. Call `send_human_review` with that exact quote_id, the same summary text, and the "
    "review_url given to you in the briefing (substituting the quote_id).\n"
    ...
    "1. Ask Restock Request Maker for a PROCEED or NEEDS_REVIEW verdict on that line. It will not "
