...
SUPERVISOR_INSTRUCTIONS = (
    ...
    "<excess_holding_cost> holding = Rs <total>] spent if <plain-language reason the spend "
    "turns out unnecessary>   <-- for a PURCHASE\n"
    "IF APPROVED AND WRONG: no money spent -- the stock is already owned; <donor warehouse> "
    "drops to <donor_cover_after_days> days of cover for nothing if <plain-language reason "
    ...
    "IF APPROVED AND WRONG must be the real, standalone consequence of executing the CHOSEN "
    "option in OPTIONS CONSIDERED. For a PURCHASE, write it left to right as a computation "
    "and let the total be the LAST thing you write: evaluate_feasibility's feasible_qty, "
    "times its unit_cost, plus excess_holding_cost if evaluate_feasibility flagged excess, "
    "then the sum. Do not decide the total first and justify it afterwards -- that is how the "
    "wrong number gets in, because a plausible-looking figure is already sitting in the "
    "ranking output. The total must equal the terms to its left; if it does not, the "
    "arithmetic is what changes, never the terms.\n"
    "For a TRANSFER there is NO money figure and you must not invent one. Nothing is "
    "purchased: the stock is already owned and only moves between warehouses, and this system "
    "has no freight or handling cost anywhere in its data. State the real downside instead -- "
    "the donor warehouse ends up thinner (donor_cover_after_days from scan_transfer_options) "
    "for no benefit. A rupee figure on a transfer line is always either action_cost in "
    "disguise or a number you back-solved from it.\n"
    "action_cost never appears as a rupee figure anywhere a PM reads -- not in this line, not "
    "in DECISION VALUE, not in OPTIONS CONSIDERED -- and neither does any per-unit rate got "
    "by dividing it or decision_value by a quantity. Those are internal ranking numbers computed for every "
    "candidate before any option is chosen -- they are not what the chosen option costs to "
    "execute. Citing one, or a rate reverse-engineered from one, is not a rounding difference: "
    "it is a fabricated number that a PM cannot check against anything, which is the single "
    "fastest way to lose their trust in every other figure here.\n\n"
    "A tool call that errors, times out, or returns nothing is NOT evidence. Never write a "
    "conclusion and attach a failed lookup to it (\"no surplus available (query failed)\" is "
    "two contradictory claims in one line). Either retry the call and report what it actually "
    "returned, or say plainly that the check could not be completed and do not lean on it -- a "
    "PM who cannot tell \"we looked and there is none\" from \"we could not look\" has no way "
    "to judge the recommendation.\n\n"
    "PLAIN LANGUAGE\n"
