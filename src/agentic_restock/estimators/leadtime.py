...
def _weights(days_ago: np.ndarray) -> np.ndarray:
    return np.power(0.5, days_ago / RECENCY_HALF_LIFE_DAYS)
...
def estimate_lead_time(
    ...
    *,
    ...
    weights = _weights(ages)

    mean_delay, sigma = _weighted_moments(delays, weights)
