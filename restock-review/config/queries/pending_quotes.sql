...
-- A quote needs PM attention if it has a line still PENDING_APPROVAL, or one
-- the fulfillment guardrail flagged NEEDS_REVIEW after approval (see
-- QuoteDetailPage.tsx -- both statuses are actionable there).
HAVING SUM(CASE WHEN drs.REQUEST_STATUS IN ('PENDING_APPROVAL', 'NEEDS_REVIEW') THEN 1 ELSE 0 END) > 0
